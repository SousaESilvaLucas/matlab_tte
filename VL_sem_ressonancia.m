function [ VL,Rl1,Rd1,L_tx,C_tx ] = VL_sem_ressonancia( mu,sigma,r,Fs )
...
% os capacitores do circuito (C_tx, C_rx, Rd1 e Rd2). Como nesta função
% geramos um canal sem ressonância, Esses valores devem ser (próximos) de 0
...
% --- Generating the Transfer Function
    ...
    %Fmic = empirical_model_70(r,w,sigma,mu);
%------------------------------ LUCAS-------------------------------------%
...
% Circuit paramesters
Vs=9.915;%Vs=50; 
Rs=50; %Rs=1e-10; %50; %Resistencia do driver de corrente (não há queda de tensão) %usar 50 se fonte de corrente sem ressonancia
RL=1e6; 
sig_copp=5.85e7;
Rd1=0;%Rd1=275; %Tx damping load (amortecimento)
Rd2=0;%Rd2=275; %Rx damping load (amortecimento)
...
Z_L = (Rl1+Rd1)+1i*w*L_tx; Z_C = 1./(1i*w*C_tx);
%Z_cur = Z_L.*Z_C./(Z_L+Z_C); 
...
%TFtext = PSpice_Transfer_Function(f, Fmic, 30); %disp(TFtext)
Md = N_Tx*S_Tx*I_L;
